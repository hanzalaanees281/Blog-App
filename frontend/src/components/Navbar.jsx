  ...
  return (
    <div className="flex items-center justify-between px-6 md:px-[200px] py-4">
      ...
      {path === "/" && <div className="flex justify-center items-center space-x-0">
        <p onClick={() => navigate(prompt?"?search="+prompt:navigate("/"))} className="cursor-pointer"><CiSearch /></p>
        <input onChange={(e) => setPrompt(e.target.value)} className="outline-none px-3" type="text" placeholder="Search a post" />

      </div>}
      <div className="hidden md:flex items-center justify-center space-x-2 md:space-x-4">
        {user ? <h3 className="hidden"><Link to="/write">Create Post</Link></h3> : <h3><Link to="/login">Login</Link></h3>}
        {user ? <div onClick={showMenu}><p className="cursor-pointer"><FaBars /></p>
          {menu && <Menu />}</div> : <h3><Link to="/signup">Signup</Link></h3>}
      </div>
